...
// In-memory store keeps file references + metadata only.
// Rows are parsed on-demand for duplicate checks and exports.
...
function parseCSVFile({ file, id, onRow, progressEvery = 100000 }) {
  return new Promise((resolve, reject) => {
    ...
    Papa.parse(file, {
      ...
      step(result) {
        ...
        rowCount++;
        onRow?.(result.data, rowCount, headers);

        if (rowCount % progressEvery === 0) {
          self.postMessage({ type: "progress", id, rowsProcessed: rowCount });
        }
      },
      complete() {
...
async function handleParseFile(msg) {
  const { id, fileId, file, raName, raNumber, raOrg } = msg;

  try {
    const { headers, rowCount } = await parseCSVFile({ file, id, progressEvery: 50000 });
...
async function handleCheckDuplicates(msg) {
  ...
  try {
    ...
    self.postMessage({ type: "result", id, error: `Duplicate check failed: ${err.message}` });
  }
}

async function handleBuildCSV(msg) {
  ...
  try {
    ...
    self.postMessage({ type: "result", id, error: `CSV build failed: ${err.message}` });
  }
}
